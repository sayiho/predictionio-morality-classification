            ...
            if line[1] == 'numeric':
                target['info'] = {
                    1: {
                        ...
                        '-1': 0,
                        'sd': 0,
                        'distribution': [0 for i in range(0, gcount + 1)]
                    },
                    0: {
                        'count': 0,
                        'max': None,
...
def statistics(file, label, out, featurestype, verbose, csv, gcount):
    ...
    head, feature_count = getHead(line, featurestype, gcount)
    ...
    for i in range(0, feature_count):
        if head[i]['type'] == 'numeric':
            info = head[i]['info']
            info['distribution'] = {
                'max': max(info[0]['max'], info[1]['max']),
                'min': min(info[0]['min'], info[1]['min'])
            }
    count = 0
    for line in file:
        line = line.rstrip('\r\n')
            ...
            if target['type'] == 'numeric':
                ...
                if x != -1:
                    ...
                    if info['max'] > info['min']:
                        idx = math.floor((x - target['info']['distribution']['min']) / ((target['info']['distribution']['max'] - target['info']['distribution']['min']) / gcount))
                        info['distribution'][idx] += 1
        count += 1
        if count % 200 == 0:
            print('deal %d lines' % count)
    for i in range(0, feature_count):
    ...
    if csv == '0':
        ...
        titles.extend(['0:distribution-%d/%d' % (i, gcount + 1) for i in range(0, gcount + 1)])
        titles.extend([
            '1:count', '1:-1', '1:avg', '1:min',
            '1:max', '1:standard deviation'
        ])
        titles.extend(['1:distribution-%d/%d' % (i, gcount + 1) for i in range(0, gcount + 1)])
        ...
        for i in range(0, feature_count):
            ...
            if target['type'] == 'numeric':
                ...
                line.extend(target['info'][0]['distribution'])
                line.extend([
                    target['info'][1]['count'],
                    target['info'][1]['-1'],
                    target['info'][1]['avg'],
                ...
                line.extend(target['info'][1]['distribution'])
